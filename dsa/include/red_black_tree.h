...
protected:
    ...
    // 借的方式非常简单，我们先把节点 C 和父节点 A 合并（上推），
    // 再把 A 分裂出来(下推)，这样 B 所在的路径的缺失的黑色就被 A 补偿了。同时，原来的 E 要替代 C 的位置变成黑色。
    // 补偿动作结束！
    //
    //                C#
    //              /    \
    ...
    //                D     E
    //
    // 这时候怎么办呢，既然兄弟穷，那就找爸爸。爸爸为了保证公平性(黑高)，得把兄弟 C 先涂红，相当于兄弟 C 也合入到 A 了
